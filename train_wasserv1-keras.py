...
mx.random.seed(5)
logger.basicConfig(level=logger.INFO, filename='logs/train_loss-w1keras.log')
...
tfs_train = gluon.data.vision.transforms.Compose([
    ...
    gluon.data.vision.transforms.RandomSaturation(0.01),
...
generator.initialize(init=mx.init.Xavier(magnitude=0.01), ctx=CTX)
discriminator.initialize(init=mx.init.Xavier(magnitude=0.01), ctx=CTX)
    ...
    for data, _ in tqdm.tqdm(
        ...
        if dis_update_time == iter4G:
            ...
            if should_use_val:
                ...
                logger.info("Generator[train: {}, val: {}]".format(1 - g_train_loss, 1 - g_val_loss))
                logger.info("Discriminator[train: {}, val: {}]".format(1 - d_train_loss, 1 - d_val_loss))
            else:
                history.update([g_train_loss, d_train_loss])
                logger.info("Generator[{}], Discriminator[{}]".format(g_train_loss, d_train_loss))

            # make a prediction
            if gen_update_time % pred_per_epoch == 0:
            ...
            history.plot(save_path='logs/historys-w1keras')
...
history.plot(save_path='logs/histories-w1keras')
