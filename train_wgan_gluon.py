...
from mxnet.gluon.data.vision import transforms, CIFAR10, ImageFolderDataset, MNIST
...
from datasets import load_rem_face
...
class DCGAN_D_nobn(nn.HybridBlock):
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0):
...
class DCGAN_G_nobn(nn.HybridBlock):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
...
parser.add_argument('--experiment', default='logs/preds', help='Where to store samples and models')
...
# setup optimizer
if opt.adam:
    trainerD = Trainer(netD.collect_params(), optimizer='adam',
                       optimizer_params={'learning_rate': opt.lrD, 'beta1': opt.beta1, 'beta2': 0.999})
    trainerG = Trainer(netG.collect_params(), optimizer='adam',
                       optimizer_params={'learning_rate': opt.lrG, 'beta1': opt.beta1, 'beta2': 0.999})
else:
    trainerD = Trainer(netD.collect_params(), optimizer='rmsprop',
                       optimizer_params={'learning_rate': opt.lrD, 'gamma1': 0.99, 'gamma2': 0.99, 'epsilon': 1e-12})
    trainerG = Trainer(netG.collect_params(), optimizer='rmsprop',
                       optimizer_params={'learning_rate': opt.lrG, 'gamma1': 0.99, 'gamma2': 0.99, 'epsilon': 1e-14})
